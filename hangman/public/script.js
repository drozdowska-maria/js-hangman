  ...
  function getmyKeyWord(b, a) {
    ...
    myKeyboard[a].addEventListener("click", function () {
      if (arr[b] === x) {
        ...
        arr[b] = arr[b].replace(
          regex,
          `<span style="${letterStyle} color:black;">${myKeyboard[a].textContent}</span>`
        );
        myKeyWord.innerHTML = arr.join("");
        ...
        if (result === true) {
          myMistakeCounter.style.backgroundColor =
        ...
        myKeyboard[a].textContent !==
        myKeyWord.textContent.charAt(
          myKeyWord.textContent.indexOf(myKeyboard[a].textContent)
        )
      ) {
